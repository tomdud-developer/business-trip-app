...
class DaysAllowanceServiceTest {
    ...
    void isDateBetweenOrEqualsTheSameDatesShouldReturnTrue() {
        Assertions.assertTrue(daysAllowanceService.isDateBetweenOrEquals(LocalDate.now(), LocalDate.now(), LocalDate.now()));
    }

    @Test
    void isDateBetweenOrEqualsOneDayAfterEndShouldReturnFalse() {
        Assertions.assertFalse(daysAllowanceService.isDateBetweenOrEquals(LocalDate.now().plusDays(2), LocalDate.now(), LocalDate.now().plusDays(1)));
    }

    @Test
    void isDateBetweenOrEqualsOneDayBeforeStartShouldReturnFalse() {
        Assertions.assertFalse(daysAllowanceService.isDateBetweenOrEquals(LocalDate.now().plusDays(-1), LocalDate.now(), LocalDate.now().plusDays(1)));
    }

    @Test
    void isDateBetweenOrEqualsEndDateBeforeStartShouldReturnFalse() {
        Assertions.assertFalse(daysAllowanceService.isDateBetweenOrEquals(LocalDate.now().plusDays(-50), LocalDate.now(), LocalDate.now().plusDays(-100)));
    }
}